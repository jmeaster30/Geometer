...
namespace Geometer.Lib.Model
{
  ...
  {
    public static IEnumerable<T> As<T>(this IEnumerable<IModel> models) where T : class
    {
      return models.Select(x => x.As<T>()).Where(x => x != null);
    }

    public static IEnumerable<PointModel> PointModels(this IEnumerable<IModel> models)
    {
      return models.Where(x => x.Type == ModelType.Point).Select(x => (PointModel)x);
    }

    public static IEnumerable<LineModel> LineModels(this IEnumerable<IModel> models)
    {
      return models.Where(x => x.Type == ModelType.Line).Select(x => (LineModel)x);
    }

    public static IEnumerable<AngleModel> AngleModels(this IEnumerable<IModel> models)
    {
      return models.Where(x => x.Type == ModelType.Angle).Select(x => (AngleModel)x);
    }

    public static IEnumerable<CircleModel> CircleModels(this IEnumerable<IModel> models)
    {
      return models.Where(x => x.Type == ModelType.Circle).Select(x => (CircleModel)x);
    }

    public static IEnumerable<PolygonModel> PolygonModels(this IEnumerable<IModel> models)
    {
      return models.Where(x => x.Type == ModelType.Polygon).Select(x => (PolygonModel)x);
    }

    public static bool MatchesASTType(this IModel model, GeometerASTType type)
    {
